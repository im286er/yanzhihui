...
namespace Manager\ORG;
     ...
     *
     * 4，支持规则表达式。
     *      在think_auth_rule 表中定义一条规则时，如果type为1， conditions字段就可以定义规则表达式。 如定义{score}>5  and {score}<100  表示用户的分数在5-100之间时这条规则才会通过。
     */
...
class Auth {
    ...
    public function check($name, $uid, $type = 1, $mode = 'url', $relation = 'or') {
        if (!$this->_config['AUTH_ON'])
            return true;
        $auth = $this->getAuthList($uid, $type); //获取用户需要验证的所有有效规则列表
        /* 遍历数据 */
        foreach ($auth as $k => $v) {
            $authList[] = $v['name'];
        }
    ...
    public function getAuthList($uid, $type) {
        ...
        //读取用户组所有权限规则
        $rules = M()->table($this->_config['AUTH_RULE'])->where($map)->field('conditions,name,menu_id')->select();
        //循环规则，判断结果。
        $authList = array();
        foreach ($rules as $rule) {
            if (!empty($rule['conditions'])) { //根据conditions进行验证
                ...
                if ($conditions) {
                    $authList[] = array('menu_id' => $rule['menu_id'], 'name' => strtolower($rule['name']));
                }
            } else {
                //只要存在就记录
                $authList[] = array('menu_id' => $rule['menu_id'], 'name' => strtolower($rule['name']));
